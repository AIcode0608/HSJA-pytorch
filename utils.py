...
def net_builder(net_name, from_name: bool, net_conf=None):
    ...
    if from_name:     #Flase
        ...
        if net_name == 'WideResNet':
            ...
            builder = getattr(net, 'build_WideResNet')()  #从nets.wrn.py中获取名称为build_wideresnet的属性   即=net.build_WideResNet
        else:
            assert Exception("Not Implemented Error")
            
        setattr_cls_from_kwargs(builder, net_conf)              #对builder即wideresnet设置net_conf的属性值，net_conf是初始化设置的网络参数。
        return builder.build                                    #返回初始化结束后的wideresnet网络属性及参数
